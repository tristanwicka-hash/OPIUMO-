...
function main(): void {
  ...
  const winners = uniq.filter((c) => c.exitProceedsSol! > c.entryProceedsSol);
  const drained = uniq.filter((c) => !(c.exitProceedsSol! > c.entryProceedsSol) && isDrained(c as any));
  const faded = uniq.filter((c) => !(c.exitProceedsSol! > c.entryProceedsSol) && !isDrained(c as any));
  ...
  say("=".repeat(112));
  say(`  ${uniq.length} closed positions: ${winners.length} winners (${((winners.length / uniq.length) * 100).toFixed(1)}%), ${drained.length} instant drains, ${faded.length} faded. Winner = exit above entry under the BOOK's pricing (superseded for Pump.fun, APPROVALS 37).`);
  say();
  ...
  const out = "reports/what-separates-2026-09-12.md"; fs.writeFileSync(out, "```\n" + L.join("\n") + "\n```\n"); console.log(`\nWritten to ${out}`);
}
main();
